...
산출된 **원점수**를 성별에 맞는 **환산표(S1~S19)**에 대입하여 **5단계(1~5)**의 상태를 판별합니다.
...
> [!NOTE]
> 상기 원점수 합계 방식은 데이터(`questions.json`)에 정의된 배점(1~4점)을 그대로 합산하여 계산합니다. A, B, C 영역 모두 **점수가 높을수록 스트레스 요인/반응이 높음(나쁨)**을 의미하도록 배점되어 있습니다.
...
```json
{
  "high_stress": true, // 고스트레스자 여부
  "scores": {
    "A": { "score": 15, "eval": 3 }, // 영역별 합산 점수
    "B": { "score": 10, "eval": 1 },
    "C": { "score": 12, "eval": 4 }
  },
  "factors": {
    "A1": { "raw": 9, "scale": 3, "eval": 3 },
    ...
  }
}
```
